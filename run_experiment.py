...
if args.param == 'both':
    ...
    real_performance_plot_SO(model_n, Y, './results/'+args.save_name, imnet_gen, 'Aspect Ratio', 'ratio', min_y[0], max_y[0], 64)
elif args.param == 'volume':
    performance_plot_SO(model, Y, './results/'+args.save_name, 'Volume Ratio')
    real_performance_plot_SO(model_n, Y, './results/'+args.save_name, imnet_gen, 'Volume Ratio', 'ratio', min_y[1], max_y[1], 64)